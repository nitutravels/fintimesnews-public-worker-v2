...
# Compose a wide newsroom shot with the presenter kept small inside the frame.
# The generated glass desk is overlaid after the presenter, which creates depth
# and avoids the flat talking-head appearance of the previous version.
ffmpeg -y \
  -loop 1 -i private/output/anchor_studio.png \
  -i "$RUNNER_TEMP/fintimes-presenter.mp4" \
  -loop 1 -i private/output/anchor_studio_foreground.png \
...
# Replace the opening part of the explainer with the studio scene while keeping
# the approved intro, disclaimer, narration and music from the base production.
ffmpeg -y \
